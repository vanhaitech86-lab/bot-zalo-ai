...
echo [1/4] Dang chuan bi thu muc dong goi: "%RELEASE_NAME%"...
if exist "%TARGET_DIR%" rd /s /q "%TARGET_DIR%"
if exist "%ZIP_FILE%" del /f /q "%ZIP_FILE%"
mkdir "%TARGET_DIR%"
mkdir "%TARGET_DIR%\api"

echo [2/4] Dang sao chep cac tap tin ma nguon va tai lieu chuan...
...
echo [3/4] Dang nen toan bo san pham thanh file ZIP thuong mai...
powershell -NoProfile -Command "Compress-Archive -Path '%TARGET_DIR%\*' -DestinationPath '%ZIP_FILE%' -Force"

echo [4/4] Dang don dep cac thu muc tam...
rd /s /q "%TARGET_DIR%"
